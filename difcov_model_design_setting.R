...
# Function for determining the MLE

log_lik_fnct_full <- function(theta, x1, x2, y00, y10, y11, w10, w11){
  
  # write out these definitions
  theta_1 <- (y00 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9])^2
  
  theta_2 <- (y10 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9] - w10*theta[10])^2
  
  theta_3 <- (y11 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9] - w11*theta[10])^2
  
  theta_4 <- 2*(y00 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9])*(y10 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9] - w10*theta[10])
  
  theta_5 <- 2*(y00 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9])*(y11 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9] - w11*theta[10])
  
  theta_6 <- 2*(y10 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9] - w10*theta[10])*(y11 - theta[5] - w10*theta[6] - w11*theta[7] - x1*theta[8] - x2*theta[9] - w11*theta[10])
  ...
  sigma23 <- theta[1]^2 + w10*w11*(theta[2]^2)
  
  sigma22 <- theta[1]^2 + theta[4]^2 + w10*(theta[2]^2)
  
  sigma33 <- theta[1]^2 + theta[3]^2 + w11*(theta[2]^2)
...
sigma0_cond_posterior <- function(sigma0, tau1, taueps, taudel, mu0, delta, y00, y10, y11, w10, w11){
  ...
  sigma23 <- sigma0 + w10*w11*(tau1^2)
  
  sigma22 <- sigma0 + taudel^2 + w10*(tau1^2)
  
  sigma33 <- sigma0 + taueps^2 + w11*(tau1^2)
...
# tau1 conditional posterior


tau1_cond_posterior <- function(tau1, sigma0, taueps, taudel, mu0, delta, y00, y10, y11, w10, w11){
  ...
  sigma23 <- sigma0^2 + w10*w11*(tau1)
  
  sigma22 <- sigma0^2 + taudel^2 + w10*(tau1)
  
  sigma33 <- sigma0^2 + taueps^2 + w11*(tau1)
  ...
  logpost <- (-1) * log( 1 + ((tau1)^2) / (25^2) ) + sum((-0.5)*log(det_Z) - 0.5*(theta_big / det_Z))
...
# taueps conditional posterior


taueps_cond_posterior <- function(taueps, sigma0, tau1, taudel, mu0, delta, y00, y10, y11, w10, w11){
  ...
  sigma23 <- sigma0^2 + w10*w11*(tau1^2)
  
  sigma22 <- sigma0^2 + taudel^2 + w10*(tau1^2)
  
  sigma33 <- sigma0^2 + taueps + w11*(tau1^2)
  ...
  logpost <- (-1) * log( 1 + ((taueps)^2) / (25^2) ) + sum((-0.5)*log(det_Z) - 0.5*(theta_big / det_Z))
...
# taudel conditional posterior


taudel_cond_posterior <- function(taudel, sigma0, tau1, taueps, mu0, delta, y00, y10, y11, w10, w11){
  ...
  sigma22 <- sigma0^2 + taudel + w10*(tau1^2)
  
  sigma33 <- sigma0^2 + taueps^2 + w11*(tau1^2)
  ...
  logpost <- (-1) * log( 1 + ((taudel)^2) / (25^2) ) + sum((-0.5)*log(det_Z) - 0.5*(theta_big / det_Z))
...
nu0_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu1, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu1_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu0, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu2_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu0, nu1, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu3_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu0, nu1, nu2, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu4_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu0, nu1, nu2, nu3, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
delta_cond_posterior <- function(sigma0, tau1, taueps, taudel, mu0, y00, y10, y11, w10, w11, v0, v1){
...
ws_cond_posterior <- function(w10, w11, sigma0, tau1, taueps, taudel, nu0, nu1, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, gam_p0, gam_p1, gam_p2, gam_q0, gam_q1, gam_q2){
...
gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4 <- matrix(NA, nrow = n_iterations, ncol = 16)
colnames(gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4) <- c("gam_p0", "gam_p1", "gam_p2", "gam_q0", "gam_q1", "gam_q2", "sigma0", "tau1", "tauepsilon", "taudelta", "nu0", "nu1", "nu2", "nu3", "nu4", "delta")

ITT_vals_model_new_Sim_A_C4 <- matrix(NA, nrow = n_iterations, ncol = n_replications)

ITT_vals_model_new_Sim_A_C4_estimates <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C4_variance <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C4_lower <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C4_upper <- matrix(NA, nrow = n_replications, ncol = n_sims)
...
for (m in 1:n_sims){
  ...
  for (h in 1:n_replications){
    ...
    while(is.null(w_draw0)){
      ...
      optim_output <- optim(par = c(1, 1, 1, 1, 0, 0, 0, 0, 0, 2),    
      ...
      if (is.na(tau1_t) | tau1_t <= 0){
        tau1_t <- runif(1, 0.5, 5)
      }
      if (is.na(taueps_t) | taueps_t <= 0){
        taueps_t <- runif(1, 0.5, 5)
      }
      if (is.na(taudel_t) | taudel_t <= 0){
        taudel_t <- runif(1, 0.5, 5)
      }
      ...
      p00 <- ws_cond_posterior(0, 0, sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p01 <- ws_cond_posterior(0, 1, sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p11 <- ws_cond_posterior(1, 1, sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p10 <- ws_cond_posterior(1, 0, sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
    ...
    for (n in 1:n_iterations){
      ...
      bvn_mu <- cbind((mu0_t + delta_t*z_assignments[,1]*imputed_matrix[,4]),
        ...
        cbind(rep((sigma0_t^2), ndat), ((sigma0_t^2) + (1-z_assignments[,1])*imputed_matrix[,4]*imputed_matrix[,5]*(tau1_t^2))) /
        ((sigma0_t^2) + z_assignments[,2]*((taudel_t^2) + imputed_matrix[,4]*(tau1_t^2)) + z_assignments[,3]*((taueps_t^2) + imputed_matrix[,5]*(tau1_t^2))) *
      ...
      lrg_sigma11[1,1,] <- (sigma0_t^2) + z_assignments[,1]*((taudel_t^2) + imputed_matrix[,4]*(tau1_t^2))
      lrg_sigma11[2,1,] <- (sigma0_t^2) + z_assignments[,1]*(imputed_matrix[,4]*imputed_matrix[,5]*(tau1_t^2))
      lrg_sigma11[1,2,] <- (sigma0_t^2) + z_assignments[,1]*(imputed_matrix[,4]*imputed_matrix[,5]*(tau1_t^2))
      lrg_sigma11[2,2,] <- (sigma0_t^2) + (1-z_assignments[,3])*((taueps_t^2) + imputed_matrix[,5]*(tau1_t^2)) + (z_assignments[,3])*((taudel_t^2) + imputed_matrix[,4]*(tau1_t^2))
      
      
      
      lrg_sigma_mult[1,1,] <- rep((sigma0_t^4), ndat) / ((sigma0_t^2) + z_assignments[,2]*((taudel_t^2) + imputed_matrix[,4]*(tau1_t^2)) + z_assignments[,3]*((taueps_t^2) + imputed_matrix[,5]*(tau1_t^2)))
      lrg_sigma_mult[2,1,] <- (sigma0_t^2) * ( (sigma0_t^2) + (1-z_assignments[,1])*imputed_matrix[,4]*imputed_matrix[,5]*(tau1_t^2) ) / ((sigma0_t^2) + z_assignments[,2]*((taudel_t^2) + imputed_matrix[,4]*(tau1_t^2)) + z_assignments[,3]*((taueps_t^2) + imputed_matrix[,5]*(tau1_t^2)))
      lrg_sigma_mult[1,2,] <- (sigma0_t^2) * ( (sigma0_t^2) + (1-z_assignments[,1])*imputed_matrix[,4]*imputed_matrix[,5]*(tau1_t^2) ) / ((sigma0_t^2) + z_assignments[,2]*((taudel_t^2) + imputed_matrix[,4]*(tau1_t^2)) + z_assignments[,3]*((taueps_t^2) + imputed_matrix[,5]*(tau1_t^2)))
      lrg_sigma_mult[2,2,] <- ( (sigma0_t^2) + (1-z_assignments[,1])*imputed_matrix[,4]*imputed_matrix[,5]*(tau1_t^2) )^2 / ((sigma0_t^2) + z_assignments[,2]*((taudel_t^2) + imputed_matrix[,4]*(tau1_t^2)) + z_assignments[,3]*((taueps_t^2) + imputed_matrix[,5]*(tau1_t^2)))
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,1] <- gam_p_t[1]
      # Then assign it to the p_t variable use throughout
      gam_p0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,1]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,2] <- gam_p_t[2]
      # Then assign it to the p_t variable use throughout
      gam_p1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,2]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,3] <- gam_p_t[3]
      # Then assign it to the p_t variable use throughout
      gam_p2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,3]
      ...
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,4] <- gam_q_t[1]
      # Then assign it to the p_t variable use throughout
      gam_q0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,4]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,5] <- gam_q_t[2]
      # Then assign it to the p_t variable use throughout
      gam_q1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,5]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,6] <- gam_q_t[3]
      # Then assign it to the p_t variable use throughout
      gam_q2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,6]
      
      
      # # Draw the next p probability and save it
      # gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,1] <- 0.7
      # # Then assign it to the p_t variable use throughout
      # p_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,1]
      # 
      # # Draw the next q probability and save it
      # gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,2] <- 0.6
      # # Then assign it to the q_t variable use throughout
      # q_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,2]
      ...
      for (d in 1:draws){
        ...
        r_top <- sigma0_cond_posterior(proposal, tau1_t, taueps_t, taudel_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- sigma0_cond_posterior(draw_d, tau1_t, taueps_t, taudel_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,7] <- sqrt(draw_d)
      
      # update the sigma0 parameter to reflect this
      sigma0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,7]
      ...
      # here we run an MH for (tau1)^2 and then transform back to tau1 at the end to keep the final draw
      for (d in 1:draws){
        
        proposal <- rtruncnorm(1, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm)
        
        r_top <- tau1_cond_posterior(proposal, sigma0_t, taueps_t, taudel_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- tau1_cond_posterior(draw_d, sigma0_t, taueps_t, taudel_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      # keep the final draw from the MH for tau1
      # must transform back from log(tau1) to tau1
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,8] <- sqrt(draw_d)
      
      # update the tau1 parameter to reflect this
      tau1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,8]
      
      
      
      
      
      # tau1_mat <- data.frame(1:draws, tau1_chain)
      # names(tau1_mat) <- c("draws", "tau1_vals")
      # ggplot(tau1_mat, aes(draws, tau1_vals)) + geom_line()
      ...
      # here we run an MH for (taueps)^2 and then transform back to taueps at the end to keep the final draw
      for (d in 1:draws){
        
        proposal <- rtruncnorm(1, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm)
        
        r_top <- taueps_cond_posterior(proposal, sigma0_t, tau1_t, taudel_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- taueps_cond_posterior(draw_d, sigma0_t, tau1_t, taudel_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      # keep the final draw from the MH for taueps
      # must transform back from log(taueps) to taueps
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,9] <- sqrt(draw_d)
      
      # update the taueps parameter to reflect this
      taueps_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,9]
      
      
      
      
      
      # taueps_mat <- data.frame(1:draws, taueps_chain)
      # names(taueps_mat) <- c("draws", "taueps_vals")
      # ggplot(taueps_mat, aes(draws, taueps_vals)) + geom_line()
      ...
      # here we run an MH for (taudel)^2 and then transform back to taudel at the end to keep the final draw
      for (d in 1:draws){
        
        proposal <- rtruncnorm(1, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm)
        
        r_top <- taudel_cond_posterior(proposal, sigma0_t, tau1_t, taueps_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- taudel_cond_posterior(draw_d, sigma0_t, tau1_t, taueps_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
      ...
      # keep the final draw from the MH for taudel
      # must transform back from log(taudel) to taudel
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,10] <- sqrt(draw_d)
      
      # update the taudel parameter to reflect this
      taudel_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,10]
      
      
      
      
      
      # taudel_mat <- data.frame(1:draws, taudel_chain)
      # names(taudel_mat) <- c("draws", "taudel_vals")
      # ggplot(taudel_mat, aes(draws, taudel_vals)) + geom_line()
      
      
      # nu0
      
      nu0_t_draw <- nu0_cond_posterior(sigma0_t, tau1_t, taueps_t, taudel_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      
      # Save the drawn nu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,11] <- nu0_t_draw
      # Then update this value
      nu0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,11]
      
      
      # nu1
      
      nu1_t_draw <- nu1_cond_posterior(sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      
      # Save the drawn nu1 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,12] <- nu1_t_draw
      # Then update this value
      nu1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,12]
      
      
      # nu2
      
      nu2_t_draw <- nu2_cond_posterior(sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu1_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      
      # Save the drawn nu2 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,13] <- nu2_t_draw
      # Then update this value
      nu2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,13]
      
      
      
      # nu 0 draw
      nu3_t_draw <- nu3_cond_posterior(sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu1_t, nu2_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,14] <- nu3_t_draw
      # Then update this value
      nu3_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,14]
      
      
      
      # nu 0 draw
      nu4_t_draw <- nu4_cond_posterior(sigma0_t, tau1_t, taueps_t, taudel_t, nu0_t, nu1_t, nu2_t, nu3_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,15] <- nu4_t_draw
      # Then update this value
      nu4_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,15]
      ...
      delta_t_draw <- delta_cond_posterior(sigma0_t, tau1_t, taueps_t, taudel_t, mu0_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], v0_delta, v1_delta)
      
      # Save the drawn delta 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,16] <- delta_t_draw
      # Then update this value
      delta_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4[n,16]
      ...
      # 
      # 
      # delta_vals_t <- rnorm(ndat, mean = delta_t, sd = tau1_t)
      # 
      # mu0_trueish <- nu0_t + w10_t*nu1_t + (1-w10_t)*w11_t*nu2_t
      # 
      # y00_t <- rnorm(ndat, mean = mu0_trueish, sd = sigma0_t)
      # 
      # y10_t <- y00_t + w10_t*delta_vals_t + rnorm(ndat, mean = 0, sd = taudel_t)
      # 
      # y11_t <- y00_t + w11_t*delta_vals_t + rnorm(ndat, mean = 0, sd = taueps_t)
      
      
      
      q_t <- mean(pnorm((gam_q0_t + x1*gam_q1_t + x2*gam_q2_t)))
      
      ITT_vals_model_new_Sim_A_C4[n,h] <- q_t * delta_t
  ...
  ITT_vals_model_new_Sim_A_C4_estimates[,m] <- apply(ITT_vals_model_new_Sim_A_C4, 2, mean)
  ITT_vals_model_new_Sim_A_C4_variance[,m] <- apply(ITT_vals_model_new_Sim_A_C4, 2, var)
  ITT_vals_model_new_Sim_A_C4_lower[,m] <- apply(ITT_vals_model_new_Sim_A_C4, 2,quantile,c(0.025))
  ITT_vals_model_new_Sim_A_C4_upper[,m] <- apply(ITT_vals_model_new_Sim_A_C4, 2,quantile,c(0.975))
  
  
  save(ITT_vals_model_new_Sim_A_C4_estimates, file = "ITT_vals_model_new_Sim_A_C4_estimates.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C4_variance, file = "ITT_vals_model_new_Sim_A_C4_variance.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C4_lower, file = "ITT_vals_model_new_Sim_A_C4_lower.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C4_upper, file = "ITT_vals_model_new_Sim_A_C4_upper.Rdata")
...
# save(gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4, file = "gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4.Rdata")
# 
# 
# save(ITT_true_vals_model_new_Sim_A_C4, file = "ITT_true_vals_model_new_Sim_A_C4.Rdata")
...
# gibbs_parameter_posteriors <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C4
# 
# itt_vals_model <- ITT_vals_model_new_Sim_A_C4
# 
# # n_iterations <- 10000
# gibbs_post_mat <- data.frame(1:n_iterations, gibbs_parameter_posteriors, itt_vals_model)
# 
# names(gibbs_post_mat) <- c("draws", "p0", "p1", "p2", "q0", "q1", "q2", "sigma0", "tau1", "taueps", "taudel", "nu0", "nu1", "nu2", "nu3", "nu4", "delta", "SP_ITT")
# 
...
# 
# ggplot(gibbs_post_mat, aes(draws, tau1)) + geom_line()
# 
# ggplot(gibbs_post_mat, aes(draws, taueps)) + geom_line()
# 
# ggplot(gibbs_post_mat, aes(draws, taudel)) + geom_line()
# 
...
# # var(ITT_vals_model_new_Sim_A_C4)
# 
# 
# 
# 
...
# # nu0 conditional posterior
# 
# nu0_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu1, nu2, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- sigma0^2
...
#   theta_A <- theta_C
#   
#   theta_B <- 2*theta_C*(x1*w10*nu1 + x2*w11*nu2) + theta_D
#   
#   mu0_sd <- sqrt( 1 / (sum(theta_A / theta_denom) + 1/k0 ) )
#   
#   mu0_mean <- (-1) * ( sum(theta_B / theta_denom) - 2*k1/k0 ) / ( 2 * ( sum(theta_A / theta_denom) + 1/k0 ) )
...
# # nu1 conditional posterior
# 
# nu1_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu0, nu2, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- sigma0^2
...
# # nu2 conditional posterior
# 
# nu2_cond_posterior <- function(sigma0, tau1, taueps, taudel, nu0, nu1, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- sigma0^2
...
# draws <- 10000
# 
# nu0_t_draw <- nu0_cond_posterior(sigma0_true, tau1_true, taueps_true, taudel_true, nu1_true, nu2_true, delta_mean, x1, x2, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
# 
# 
...
# # nu1
# 
# nu1_t_draw <- nu1_cond_posterior(sigma0_true, tau1_true, taueps_true, taudel_true, nu0_true, nu2_true, delta_mean, x1, x2, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# # nu2
# 
# nu2_t_draw <- nu2_cond_posterior(sigma0_true, tau1_true, taueps_true, taudel_true, nu0_true, nu1_true, delta_mean, x1, x2, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# # delta
# delta_t_draw <- delta_cond_posterior(sigma0_true, tau1_true, taueps_true, taudel_true, mu0_true, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
# 
