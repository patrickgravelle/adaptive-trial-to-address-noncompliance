...
log_lik_fnct_full <- function(theta, x1, x2, y00, y10, y11, w10, w11){
  ...
  theta_1 <- (y00 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] )^2
  
  theta_2 <- (y10 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w10*theta[9])^2
  
  theta_3 <- (y11 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w11*theta[9])^2
  
  theta_4 <- 2*(y00 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8])*(y10 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w10*theta[9])
  
  theta_5 <- 2*(y00 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8])*(y11 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w11*theta[9])
  
  theta_6 <- 2*(y10 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w10*theta[9])*(y11 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w11*theta[9])
  
  
  sig10 <- w10*(theta[2]^2) + (1-w10)*theta[1]^2
  
  sig11 <- w11*(theta[2]^2) + (1-w11)*theta[1]^2
  
  
  sigma11 <- theta[1]^2
  
  sigma12 <- theta[3]*theta[1]*sqrt(sig10)
  
  sigma13 <- theta[3]*theta[1]*sqrt(sig11)
  
  sigma23 <- theta[3]*sqrt(sig10)*sqrt(sig11)
...
sigma0_cond_posterior <- function(sigma0, sigma1, rho, mu0, delta, y00, y10, y11, w10, w11){
  ...
  sig10 <- w10*(sigma1^2) + (1-w10)*sigma0
  
  sig11 <- w11*(sigma1^2) + (1-w11)*sigma0
  
  
  sigma11 <- sigma0
  
  sigma12 <- rho*sqrt(sigma0)*sqrt(sig10)
  
  sigma13 <- rho*sqrt(sigma0)*sqrt(sig11)
  
  sigma23 <- rho*sqrt(sig10)*sqrt(sig11)
...
sigma1_cond_posterior <- function(sigma1, sigma0, rho, mu0, delta, y00, y10, y11, w10, w11){
  ...
  sig10 <- w10*sigma1 + (1-w10)*(sigma0^2)
  
  sig11 <- w11*sigma1 + (1-w11)*(sigma0^2)
  
  
  sigma11 <- sigma0^2
  
  sigma12 <- rho*sigma0*sqrt(sig10)
  
  sigma13 <- rho*sigma0*sqrt(sig11)
  ...
  logpost <- (-1) * log( 1 + ((sigma1)^2) / (25^2) ) + sum((-0.5)*log(det_Z) - 0.5*(theta_big / det_Z))
...
rho_cond_posterior <- function(rho, sigma0, sigma1, mu0, delta, y00, y10, y11, w10, w11){
  ...
  sig10 <- w10*(sigma1^2) + (1-w10)*(sigma0^2)
  
  sig11 <- w11*(sigma1^2) + (1-w11)*(sigma0^2)
...
nu0_cond_posterior <- function(sigma0, sigma1, rho, nu1, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu1_cond_posterior <- function(sigma0, sigma1, rho, nu0, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu2_cond_posterior <- function(sigma0, sigma1, rho, nu0, nu1, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu3_cond_posterior <- function(sigma0, sigma1, rho, nu0, nu1, nu2, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu4_cond_posterior <- function(sigma0, sigma1, rho, nu0, nu1, nu2, nu3, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
delta_cond_posterior <- function(sigma0, sigma1, rho, mu0, y00, y10, y11, w10, w11, v0, v1){
...
ws_cond_posterior <- function(w10, w11, sigma0, sigma1, rho, nu0, nu1, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, gam_p0, gam_p1, gam_p2, gam_q0, gam_q1, gam_q2){
...
gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3 <- matrix(NA, nrow = n_iterations, ncol = 15)
colnames(gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3) <- c("gam_p0", "gam_p1", "gam_p2", "gam_q0", "gam_q1", "gam_q2", "sigma0", "sigma1", "rho", "nu0", "nu1", "nu2", "nu3", "nu4", "delta")

ITT_vals_model_new_Sim_A_C3 <- matrix(NA, nrow = n_iterations, ncol = n_replications)

ITT_vals_model_new_Sim_A_C3_estimates <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C3_variance <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C3_lower <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C3_upper <- matrix(NA, nrow = n_replications, ncol = n_sims)
...
for (m in 1:n_sims){
  ...
  for (h in 1:n_replications){
    ...
    while(is.null(w_draw0)){
      ...
      optim_output <- optim(par = c(1, 1, 0.5, 0, 0, 0, 0, 0, 1),    
      ...
      if (is.na(sigma1_t) | sigma1_t <= 0){
        sigma1_t <- runif(1, 0.5, 5)
      }
      ...
      p00 <- ws_cond_posterior(0, 0, sigma0_t, sigma1_t, rho_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p01 <- ws_cond_posterior(0, 1, sigma0_t, sigma1_t, rho_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p11 <- ws_cond_posterior(1, 1, sigma0_t, sigma1_t, rho_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p10 <- ws_cond_posterior(1, 0, sigma0_t, sigma1_t, rho_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
    ...
    for (n in 1:n_iterations){
      ...
      sig10_t <- imputed_matrix[,4]*sigma1_t^2 + (1-imputed_matrix[,4])*sigma0_t^2
      
      sig11_t <- imputed_matrix[,5]*sigma1_t^2 + (1-imputed_matrix[,5])*sigma0_t^2
      ...
      bvn_mu <- cbind((mu0_t + delta_t*z_assignments[,1]*imputed_matrix[,4]),
        ...
        ( cbind(rho_t*sigma0_t*( (1-z_assignments[,3])*sqrt(sig10_t) + z_assignments[,3]*sqrt(sig11_t) ), 
                rho_t*sqrt(sig11_t)*( (1-z_assignments[,1])*sqrt(sig10_t) + z_assignments[,1]*sigma0_t )) /
            (z_assignments[,1]*sigma0_t^2 + z_assignments[,2]*sig10_t + z_assignments[,3]*sig11_t) * 
      ...
      lrg_sigma11[1,1,] <- (1 - z_assignments[,1])*sigma0_t^2 + z_assignments[,1]*sig10_t
      lrg_sigma11[2,1,] <- z_assignments[,1]*rho_t*sqrt(sig10_t)*sqrt(sig11_t) + z_assignments[,2]*rho_t*sigma0_t*sqrt(sig11_t) + z_assignments[,3]*rho_t*sigma0_t*sqrt(sig10_t)
      lrg_sigma11[1,2,] <- z_assignments[,1]*rho_t*sqrt(sig10_t)*sqrt(sig11_t) + z_assignments[,2]*rho_t*sigma0_t*sqrt(sig11_t) + z_assignments[,3]*rho_t*sigma0_t*sqrt(sig10_t)
      lrg_sigma11[2,2,] <- (1 - z_assignments[,3])*sig11_t + z_assignments[,3]*sig10_t
      
      
      
      lrg_sigma_mult[1,1,] <- (rho_t*sigma0_t*((1-z_assignments[,3])*sqrt(sig10_t) + z_assignments[,3]*sqrt(sig11_t)))^2 / (z_assignments[,1]*sigma0_t^2 + z_assignments[,2]*sig10_t + z_assignments[,3]*sig11_t)
      lrg_sigma_mult[2,1,] <- (rho_t*sigma0_t*( (1-z_assignments[,3])*sqrt(sig10_t) + z_assignments[,3]*sqrt(sig11_t) )) * 
        (rho_t*sqrt(sig11_t)*( (1-z_assignments[,1])*sqrt(sig10_t) + z_assignments[,1]*sigma0_t )) / (z_assignments[,1]*sigma0_t^2 + z_assignments[,2]*sig10_t + z_assignments[,3]*sig11_t)
      lrg_sigma_mult[1,2,] <- (rho_t*sigma0_t*( (1-z_assignments[,3])*sqrt(sig10_t) + z_assignments[,3]*sqrt(sig11_t) )) * 
        (rho_t*sqrt(sig11_t)*( (1-z_assignments[,1])*sqrt(sig10_t) + z_assignments[,1]*sigma0_t )) / (z_assignments[,1]*sigma0_t^2 + z_assignments[,2]*sig10_t + z_assignments[,3]*sig11_t)
      lrg_sigma_mult[2,2,] <- (rho_t*sqrt(sig11_t)*( (1-z_assignments[,1])*sqrt(sig10_t) + z_assignments[,1]*sigma0_t ))^2 / (z_assignments[,1]*sigma0_t^2 + z_assignments[,2]*sig10_t + z_assignments[,3]*sig11_t)
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,1] <- gam_p_t[1]
      # Then assign it to the p_t variable use throughout
      gam_p0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,1]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,2] <- gam_p_t[2]
      # Then assign it to the p_t variable use throughout
      gam_p1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,2]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,3] <- gam_p_t[3]
      # Then assign it to the p_t variable use throughout
      gam_p2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,3]
      ...
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,4] <- gam_q_t[1]
      # Then assign it to the p_t variable use throughout
      gam_q0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,4]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,5] <- gam_q_t[2]
      # Then assign it to the p_t variable use throughout
      gam_q1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,5]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,6] <- gam_q_t[3]
      # Then assign it to the p_t variable use throughout
      gam_q2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,6]
      
      
      # # Draw the next p probability and save it
      # gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,1] <- 0.7
      # # Then assign it to the p_t variable use throughout
      # p_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,1]
      # 
      # # Draw the next q probability and save it
      # gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,2] <- 0.6
      # # Then assign it to the q_t variable use throughout
      # q_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,2]
      ...
      for (d in 1:draws){
        ...
        r_top <- sigma0_cond_posterior(proposal, sigma1_t, rho_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- sigma0_cond_posterior(draw_d, sigma1_t, rho_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,7] <- sqrt(draw_d)
      
      # update the sigma0 parameter to reflect this
      sigma0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,7]
      ...
      # here we run an MH for (sigma1)^2 and then transform back to sigma1 at the end to keep the final draw
      for (d in 1:draws){
        
        proposal <- rtruncnorm(1, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm)
        
        r_top <- sigma1_cond_posterior(proposal, sigma0_t, rho_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- sigma1_cond_posterior(draw_d, sigma0_t, rho_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      # keep the final draw from the MH for sigma1
      # must transform back from log(sigma1) to sigma1
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,8] <- sqrt(draw_d)
      
      # update the sigma1 parameter to reflect this
      sigma1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,8]
      
      
      
      
      
      # sigma1_mat <- data.frame(1:draws, sigma1_chain)
      # names(sigma1_mat) <- c("draws", "sigma1_vals")
      # ggplot(sigma1_mat, aes(draws, sigma1_vals)) + geom_line()
      ...
      for (d in 1:draws){
        ...
        if (proposal < 0.9999){
          r_top <- rho_cond_posterior(proposal, sigma0_t, sigma1_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
          
          r_bottom <- rho_cond_posterior(draw_d, sigma0_t, sigma1_t, mu0_t, delta_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,9] <- draw_d
      
      # update the rho parameter to reflect this
      rho_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,9]
      ...
      # nu 0 draw
      nu0_t_draw <- nu0_cond_posterior(sigma0_t, sigma1_t, rho_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,10] <- nu0_t_draw
      # Then update this value
      nu0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,10]
      
      
      # nu1 draw
      nu1_t_draw <- nu1_cond_posterior(sigma0_t, sigma1_t, rho_t, nu0_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,11] <- nu1_t_draw
      # Then update this value
      nu1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,11]
      
      
      # nu 0 draw
      nu2_t_draw <- nu2_cond_posterior(sigma0_t, sigma1_t, rho_t, nu0_t, nu1_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,12] <- nu2_t_draw
      # Then update this value
      nu2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,12]
      
      
      
      
      # nu 0 draw
      nu3_t_draw <- nu3_cond_posterior(sigma0_t, sigma1_t, rho_t, nu0_t, nu1_t, nu2_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,13] <- nu3_t_draw
      # Then update this value
      nu3_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,13]
      
      
      
      # nu 0 draw
      nu4_t_draw <- nu4_cond_posterior(sigma0_t, sigma1_t, rho_t, nu0_t, nu1_t, nu2_t, nu3_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,14] <- nu4_t_draw
      # Then update this value
      nu4_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,14]
      ...
      # delta draw
      delta_t_draw <- delta_cond_posterior(sigma0_t, sigma1_t, rho_t, mu0_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn delta 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,15] <- delta_t_draw
      # Then update this value
      delta_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3[n,15]
      ...
      # p_t <- mean(pnorm((gam_p0_t + x1*gam_p1_t + x2*gam_p2_t)))
      # 
      # q_t <- mean(pnorm((gam_q0_t + x1*gam_q1_t + x2*gam_q2_t)))
      # 
      # ITT_vals_model_new_Sim_A_C3_ITT[n,h] <- q_t * delta_t
      
      
      q_t <- mean(pnorm((gam_q0_t + x1*gam_q1_t + x2*gam_q2_t)))
      
      ITT_vals_model_new_Sim_A_C3[n,h] <- q_t * delta_t
  ...
  ITT_vals_model_new_Sim_A_C3_estimates[,m] <- apply(ITT_vals_model_new_Sim_A_C3, 2, mean)
  ITT_vals_model_new_Sim_A_C3_variance[,m] <- apply(ITT_vals_model_new_Sim_A_C3, 2, var)
  ITT_vals_model_new_Sim_A_C3_lower[,m] <- apply(ITT_vals_model_new_Sim_A_C3, 2,quantile,c(0.025))
  ITT_vals_model_new_Sim_A_C3_upper[,m] <- apply(ITT_vals_model_new_Sim_A_C3, 2,quantile,c(0.975))
  
  
  save(ITT_vals_model_new_Sim_A_C3_estimates, file = "ITT_vals_model_new_Sim_A_C3_estimates.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C3_variance, file = "ITT_vals_model_new_Sim_A_C3_variance.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C3_lower, file = "ITT_vals_model_new_Sim_A_C3_lower.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C3_upper, file = "ITT_vals_model_new_Sim_A_C3_upper.Rdata")
  
  
}


# test_mat <- matrix(NA, nrow = n_replications, ncol = n_sims)
# 
# test_mat[,m] <- apply(ITT_vals_model_new_Sim_A_C3, 2, mean)
# 
# itt_est <- apply(ITT_vals_model_new_Sim_A_C3, 2, mean)
# 
# itt_VW <- apply(ITT_within_var_new_Sim_A_C3, 2, mean)
# 
# itt_VB <- apply(ITT_vals_model_new_Sim_A_C3, 2, var)
# 
# 
# itt_var <- itt_VW + itt_VB + itt_VB/nrow(ITT_vals_model_new_Sim_A_C3)
# 
# 
# sqrt(itt_var)
# 
# 
# vm_dof <- (nrow(ITT_vals_model_new_Sim_A_C3) - 1)*((1 + (itt_VW / (itt_VB + itt_VB/nrow(ITT_vals_model_new_Sim_A_C3))))^2)
# 
# tdist_vm <- qt(0.975, vm_dof)
# 
# 
# itt_lower <- itt_est - tdist_vm*sqrt(itt_var)
# itt_upper <- itt_est + tdist_vm*sqrt(itt_var)
# 
# itt_coverage <- ifelse((rep(true_data_new_simulation_Sim_A_MI_SingleConfig_SepComp[1],50) <= itt_upper &
#                           rep(true_data_new_simulation_Sim_A_MI_SingleConfig_SepComp[1],50) >= itt_lower) , 1, 0)
# 
# 
# mean(itt_coverage)

# itt_est[which(itt_coverage == 0)]
# 
# itt_upper[which(itt_coverage == 0)]
# 
# itt_lower[which(itt_coverage == 0)]
# 
# sum(itt_coverage)
# 
# mean(abs(rep(true_data_new_simulation_Sim_A_MI_SingleConfig_SepComp[1],50) - itt_est))
# 
# mean(rep(true_data_new_simulation_Sim_A_MI_SingleConfig_SepComp[1],50) - itt_est)
# 
# mean(itt_var)
# 
# 
# 
# # Super population expectation Credible Interval
# 
# itt_est_sp <- apply(ITT_vals_model_new_Sim_A_C3_ITT, 2, mean)
# 
# itt_lower_sp <- apply(ITT_vals_model_new_Sim_A_C3_ITT, 2, quantile,c(0.025))
# 
# itt_upper_sp <- apply(ITT_vals_model_new_Sim_A_C3_ITT, 2, quantile,c(0.975))
# 
# itt_coverage_sp <- ifelse((rep(true_data_new_simulation_Sim_A_MI_SingleConfig_SepComp[1],50) <= itt_upper_sp &
#                           rep(true_data_new_simulation_Sim_A_MI_SingleConfig_SepComp[1],50) >= itt_lower_sp) , 1, 0)
# 
# 
# mean(itt_coverage_sp)
# 
# which(itt_coverage_sp == 0)



# # save(gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3, file = "gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3.Rdata")
# # 
# # 
# # save(ITT_true_vals_model_new_Sim_A_C3, file = "ITT_true_vals_model_new_Sim_A_C3.Rdata")
# 
# 
# 
# 
# 
# 
# gibbs_parameter_posteriors <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C3
# 
# itt_vals_model <- ITT_vals_model_new_Sim_A_C3
# 
# # n_iterations <- 10000
# gibbs_post_mat <- data.frame(1:n_iterations, gibbs_parameter_posteriors, itt_vals_model)
# 
# names(gibbs_post_mat) <- c("draws", "p0", "p1", "p2", "q0", "q1", "q2", "sigma0", "sigma1", "rho", "nu0", "nu1", "nu2", "nu3", "nu4", "delta", "SP_ITT")
# 
...
# 
# ggplot(gibbs_post_mat, aes(draws, sigma1)) + geom_line()
# 
...
# # var(ITT_vals_model_new_Sim_A_C3)
# 
# 
# 
# 
...
# # nu0 conditional posterior
# 
# nu0_cond_posterior <- function(sigma0, sigma1, rho, nu1, nu2, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
#   
#   sig10 <- w10*sigma1^2 + (1-w10)*sigma0^2
#   
#   sig11 <- w11*sigma1^2 + (1-w11)*sigma0^2
...
# # nu1 conditional posterior
# 
# nu1_cond_posterior <- function(sigma0, sigma1, rho, nu0, nu2, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
#   
#   sig10 <- w10*sigma1^2 + (1-w10)*sigma0^2
#   
#   sig11 <- w11*sigma1^2 + (1-w11)*sigma0^2
...
# # nu2 conditional posterior
# 
# nu2_cond_posterior <- function(sigma0, sigma1, rho, nu0, nu1, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
#   
#   sig10 <- w10*sigma1^2 + (1-w10)*sigma0^2
#   
#   sig11 <- w11*sigma1^2 + (1-w11)*sigma0^2
...
# draws <- 10000
# 
# nu0_t_draw <- nu0_cond_posterior(sigma0_true, sigma1_true, rho_true, nu1_true, nu2_true, delta_mean, x1, x2, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# # nu1
# 
# nu1_t_draw <- nu1_cond_posterior(sigma0_true, sigma1_true, rho_true, nu0_true, nu2_true, delta_mean, x1, x2, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# # nu1
# 
# nu2_t_draw <- nu2_cond_posterior(sigma0_true, sigma1_true, rho_true, nu0_true, nu1_true, delta_mean, x1, x2, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# # delta
# delta_t_draw <- delta_cond_posterior(sigma0_true, sigma1_true, rho_true, mu0_true, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, v0_delta, v1_delta)
# 
# 
