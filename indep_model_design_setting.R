...
log_lik_fnct_full <- function(theta, x1, x2, y00, y10, y11, w10, w11){
  ...
  logpost <- sum((-0.5)*log(theta[1]) - 0.5*( ((y00 - theta[4] - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8])^2) / theta[1]^2)) +
    sum( (-0.5)*log(theta[2]) - 0.5*( ( (y10 - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w10*theta[9] )^2 ) / theta[2]^2) ) +
    sum( (-0.5)*log(theta[3]) - 0.5*( ( (y11 - w10*theta[5] - w11*theta[6] - x1*theta[7] - x2*theta[8] - w11*theta[9] )^2 ) / theta[3]^2) )
...
sigma0_cond_posterior <- function(sigma0, mu0, y00){
  
  logpost <- (-1) * log( 1 + ((sigma0)^2) / (25^2) ) + sum((-0.5)*log(sigma0) - 0.5*( ((y00 - mu0)^2) / sigma0))
...
sigma10_cond_posterior <- function(sigma10, mu0, delta, y10, w10){
  
  logpost <- (-1) * log( 1 + ((sigma10)^2) / (25^2) ) + sum( (-0.5)*log(sigma10) - 0.5*( ( (y10 - (mu0 + w10*delta) )^2 ) / sigma10) )
...
sigma11_cond_posterior <- function(sigma11, mu0, delta, y11, w11){
  
  logpost <- (-1) * log( 1 + ((sigma11)^2) / (25^2) ) + sum( (-0.5)*log(sigma11) - 0.5*( ( (y11 - (mu0 + w11*delta) )^2 ) / sigma11) )
...
nu0_cond_posterior <- function(sigma0, sigma10, sigma11, nu1, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu1_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu2_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu1, nu3, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu3_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu1, nu2, nu4, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
nu4_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu1, nu2, nu3, delta, x1, x2, y00, y10, y11, w10, w11, k0, k1){
...
delta_cond_posterior <- function(sigma0, sigma10, sigma11, mu0, y00, y10, y11, w10, w11, v0, v1){
...
ws_cond_posterior <- function(w10, w11, sigma0, sigma10, sigma11, nu0, nu1, nu2, nu3, nu4, delta, x1, x2, y00, y10, y11, gam_p0, gam_p1, gam_p2, gam_q0, gam_q1, gam_q2){
...
gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1 <- matrix(NA, nrow = n_iterations, ncol = 15)
colnames(gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1) <- c("gam_p0", "gam_p1", "gam_p2", "gam_q0", "gam_q1", "gam_q2", "sigma0", "sigma10", "sigma11", "nu0", "nu1", "nu2", "nu3", "nu4", "delta")

ITT_vals_model_new_Sim_A_C1 <- matrix(NA, nrow = n_iterations, ncol = n_replications)

ITT_vals_model_new_Sim_A_C1_estimates <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C1_variance <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C1_lower <- matrix(NA, nrow = n_replications, ncol = n_sims)
ITT_vals_model_new_Sim_A_C1_upper <- matrix(NA, nrow = n_replications, ncol = n_sims)
...
for (m in 1:n_sims){
  ...
  for (h in 1:n_replications){
    ...
    while(is.null(w_draw0)){
      ...
      optim_output <- optim(par = c(1, 1, 1, 0, 0, 0, 0, 0, 2),    
      ...
      if (is.na(sigma10_t) | sigma10_t <= 0){
        sigma10_t <- runif(1, 0.5, 5)
      }
      if (is.na(sigma11_t) | sigma11_t <= 0){
        sigma11_t <- runif(1, 0.5, 5)
      }
      ...
      p00 <- ws_cond_posterior(0, 0, sigma0_t, sigma10_t, sigma11_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p01 <- ws_cond_posterior(0, 1, sigma0_t, sigma10_t, sigma11_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p11 <- ws_cond_posterior(1, 1, sigma0_t, sigma10_t, sigma11_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
      
      p10 <- ws_cond_posterior(1, 0, sigma0_t, sigma10_t, sigma11_t, nu0_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2,
                               imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], gam_p0_t, gam_p1_t, gam_p2_t, gam_q0_t, gam_q1_t, gam_q2_t)
    ...
    for (n in 1:n_iterations){
      ...
      ymis00 <- rnorm(ndat, mean = mu0_t, sigma0_t)
      
      ymis10 <- rnorm(ndat, mean = (mu0_t + imputed_matrix[,4]*delta_t), sd = sigma10_t)
      
      ymis11 <- rnorm(ndat, mean = (mu0_t + imputed_matrix[,5]*delta_t), sd = sigma11_t)
      
      
      # place the missing values into the correct cells
      imputed_matrix[,1] <- z_assignments[,1] * imputed_matrix[,1] + (1 - z_assignments[,1]) * ymis00
      
      imputed_matrix[,2] <- z_assignments[,2] * imputed_matrix[,2] + (1 - z_assignments[,2]) * ymis10
      
      imputed_matrix[,3] <- z_assignments[,3] * imputed_matrix[,3] + (1 - z_assignments[,3]) * ymis11
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,1] <- gam_p_t[1]
      # Then assign it to the p_t variable use throughout
      gam_p0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,1]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,2] <- gam_p_t[2]
      # Then assign it to the p_t variable use throughout
      gam_p1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,2]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,3] <- gam_p_t[3]
      # Then assign it to the p_t variable use throughout
      gam_p2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,3]
      ...
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,4] <- gam_q_t[1]
      # Then assign it to the p_t variable use throughout
      gam_q0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,4]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,5] <- gam_q_t[2]
      # Then assign it to the p_t variable use throughout
      gam_q1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,5]
      
      
      # save the new parameters
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,6] <- gam_q_t[3]
      # Then assign it to the p_t variable use throughout
      gam_q2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,6]
      
      
      # # Draw the next p probability and save it
      # gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,1] <- 0.7
      # # Then assign it to the p_t variable use throughout
      # p_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,1]
      # 
      # # Draw the next q probability and save it
      # gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,2] <- 0.6
      # # Then assign it to the q_t variable use throughout
      # q_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,2]
      ...
      for (d in 1:draws){
        ...
        r_top <- sigma0_cond_posterior(proposal, mu0_t, imputed_matrix[,1]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- sigma0_cond_posterior(draw_d, mu0_t, imputed_matrix[,1]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,7] <- sqrt(draw_d)
      
      # update the sigma0 parameter to reflect this
      sigma0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,7]
      ...
      # here we run an MH for (sigma10)^2 and then transform back to sigma10 at the end to keep the final draw
      for (d in 1:draws){
        
        proposal <- rtruncnorm(1, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm)
        
        r_top <- sigma10_cond_posterior(proposal, mu0_t, delta_t, imputed_matrix[,2], imputed_matrix[,4]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- sigma10_cond_posterior(draw_d, mu0_t, delta_t, imputed_matrix[,2], imputed_matrix[,4]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,8] <- sqrt(draw_d)
      
      # update the sigma1 parameter to reflect this
      sigma10_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,8]
      ...
      # here we run an MH for (sigma11)^2 and then transform back to sigma11 at the end to keep the final draw
      for (d in 1:draws){
        
        proposal <- rtruncnorm(1, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm)
        
        r_top <- sigma11_cond_posterior(proposal, mu0_t, delta_t, imputed_matrix[,3], imputed_matrix[,5]) - log(dtruncnorm(proposal, a = 0.0001, mean = draw_d, sd = 2.4*sig_norm))
        
        r_bottom <- sigma11_cond_posterior(draw_d, mu0_t, delta_t, imputed_matrix[,3], imputed_matrix[,5]) - log(dtruncnorm(draw_d, a = 0.0001, mean = proposal, sd = 2.4*sig_norm))
      ...
      # keep the final draw from the MH for sigma11
      # must transform back from log(sigma11) to sigma11
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,9] <- sqrt(draw_d)
      
      # update the sigma11 parameter to reflect this
      sigma11_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,9]
      
      
      
      # sigma11_mat <- data.frame(1:draws, sigma11_chain)
      # names(sigma11_mat) <- c("draws", "sigma11_vals")
      # ggplot(sigma11_mat, aes(draws, sigma11_vals)) + geom_line()
      
      
      
      # nu 0 draw
      nu0_t_draw <- nu0_cond_posterior(sigma0_t, sigma10_t, sigma11_t, nu1_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,10] <- nu0_t_draw
      # Then update this value
      nu0_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,10]
      
      
      # nu1 draw
      nu1_t_draw <- nu1_cond_posterior(sigma0_t, sigma10_t, sigma11_t, nu0_t, nu2_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,11] <- nu1_t_draw
      # Then update this value
      nu1_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,11]
      
      
      # nu 0 draw
      nu2_t_draw <- nu2_cond_posterior(sigma0_t, sigma10_t, sigma11_t, nu0_t, nu1_t, nu3_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,12] <- nu2_t_draw
      # Then update this value
      nu2_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,12]
      
      
      
      # nu 0 draw
      nu3_t_draw <- nu3_cond_posterior(sigma0_t, sigma10_t, sigma11_t, nu0_t, nu1_t, nu2_t, nu4_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,13] <- nu3_t_draw
      # Then update this value
      nu3_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,13]
      
      
      
      # nu 0 draw
      nu4_t_draw <- nu4_cond_posterior(sigma0_t, sigma10_t, sigma11_t, nu0_t, nu1_t, nu2_t, nu3_t, delta_t, x1, x2, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn mu0 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,14] <- nu4_t_draw
      # Then update this value
      nu4_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,14]
      ...
      # delta draw
      delta_t_draw <- delta_cond_posterior(sigma0_t, sigma10_t, sigma11_t, mu0_t, imputed_matrix[,1], imputed_matrix[,2], imputed_matrix[,3], imputed_matrix[,4], imputed_matrix[,5], k0_mu0, k1_mu0)
      
      # Save the drawn delta 
      gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,15] <- delta_t_draw
      # Then update this value
      delta_t <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1[n,15]
      ...
      ITT_vals_model_new_Sim_A_C1[n,h] <- q_t * delta_t
  ...
  ITT_vals_model_new_Sim_A_C1_estimates[,m] <- apply(ITT_vals_model_new_Sim_A_C1, 2, mean)
  ITT_vals_model_new_Sim_A_C1_variance[,m] <- apply(ITT_vals_model_new_Sim_A_C1, 2, var)
  ITT_vals_model_new_Sim_A_C1_lower[,m] <- apply(ITT_vals_model_new_Sim_A_C1, 2,quantile,c(0.025))
  ITT_vals_model_new_Sim_A_C1_upper[,m] <- apply(ITT_vals_model_new_Sim_A_C1, 2,quantile,c(0.975))
  
  
  save(ITT_vals_model_new_Sim_A_C1_estimates, file = "ITT_vals_model_new_Sim_A_C1_estimates.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C1_variance, file = "ITT_vals_model_new_Sim_A_C1_variance.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C1_lower, file = "ITT_vals_model_new_Sim_A_C1_lower.Rdata")
  
  save(ITT_vals_model_new_Sim_A_C1_upper, file = "ITT_vals_model_new_Sim_A_C1_upper.Rdata")
...
# save(gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1, file = "gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1.Rdata")
# 
# 
# save(ITT_true_vals_model_new_Sim_A_C1, file = "ITT_true_vals_model_new_Sim_A_C1.Rdata")





# gibbs_parameter_posteriors <- gibbs_parameter_posteriors_vectorized_model_new_Sim_A_C1
# 
# itt_vals_model <- ITT_vals_model_new_Sim_A_C1
# 
# # n_iterations <- 10000
# gibbs_post_mat <- data.frame(1:n_iterations, gibbs_parameter_posteriors, itt_vals_model)
# 
# names(gibbs_post_mat) <- c("draws", "p0", "p1", "p2", "q0", "q1", "q2", "sigma0", "sigma10", "sigma11", "nu0", "nu1", "nu2", "nu3", "nu4", "delta", "SP_ITT")
# 
...
# 
# ggplot(gibbs_post_mat, aes(draws, sigma10)) + geom_line()
# 
# ggplot(gibbs_post_mat, aes(draws, sigma11)) + geom_line()
# 
...
# # var(ITT_vals_model_A2)
# 
# # table(observed_outcome_data[,5])
# 
# 
# 
# draws <- 10000
...
# # nu0 conditional posterior
# 
# nu0_cond_posterior <- function(sigma0, sigma10, sigma11, nu1, nu2, nu3, nu4, delta, x1, x2, x3, x4, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- 0
...
#   theta_A <- theta_C
#   
#   theta_B <- 2*theta_C*(x1*w10*nu1 + x2*w11*nu2 + x3*nu3 + x4*nu4) + theta_D
#   
#   mu0_sd <- sqrt( 1 / (ndat*(theta_A / theta_denom) + 1/k0 ) )
#   
#   mu0_mean <- (-1) * ( sum(theta_B / theta_denom) - 2*k1/k0 ) / ( 2 * ( ndat*(theta_A / theta_denom) + 1/k0 ) )
...
# # nu1 conditional posterior
# 
# nu1_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu2, nu3, nu4, delta, x1, x2, x3, x4, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- 0
...
#   
#   theta_B <- 2*theta_C*x1*w10*(nu0 + x2*w11*nu2 + x3*nu3 + x4*nu4) + x1*w10*theta_D
#   
#   mu0_sd <- sqrt( 1 / (sum(theta_A / theta_denom) + 1/k0 ) )
#   
#   mu0_mean <- (-1) * ( sum(theta_B / theta_denom) - 2*k1/k0 ) / ( 2 * ( sum(theta_A / theta_denom) + 1/k0 ) )
...
# # nu2 conditional posterior
# 
# nu2_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu1, nu3, nu4, delta, x1, x2, x3, x4, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- 0
...
#   
#   theta_B <- 2*theta_C*x2*w11*(nu0 + x1*w10*nu1 + x3*nu3 + x4*nu4) + x2*w11*theta_D
#   
#   mu0_sd <- sqrt( 1 / (sum(theta_A / theta_denom) + 1/k0 ) )
#   
#   mu0_mean <- (-1) * ( sum(theta_B / theta_denom) - 2*k1/k0 ) / ( 2 * ( sum(theta_A / theta_denom) + 1/k0 ) )
...
# # nu3 conditional posterior
# 
# nu3_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu1, nu2, nu4, delta, x1, x2, x3, x4, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- 0
...
#                      gamma_1*w10*delta + gamma_2*w11*delta + gamma_3*w10*delta + gamma_4*w11*delta + gamma_5*w10*delta + gamma_5*w11*delta )
#   
#   theta_A <- (x3^2)*theta_C
#   
#   theta_B <- 2*theta_C*x3*(nu0 + x1*w10*nu1 + x2*w11*nu2 + x4*nu4) + x3*theta_D
#   
#   mu0_sd <- sqrt( 1 / (sum(theta_A / theta_denom) + 1/k0 ) )
#   
#   mu0_mean <- (-1) * ( sum(theta_B / theta_denom) - 2*k1/k0 ) / ( 2 * ( sum(theta_A / theta_denom) + 1/k0 ) )
...
# # nu4 conditional posterior
# 
# nu4_cond_posterior <- function(sigma0, sigma10, sigma11, nu0, nu1, nu2, nu3, delta, x1, x2, x3, x4, y00, y10, y11, w10, w11, k0, k1){
#   
#   sigma11 <- sigma0^2
#   
#   sigma12 <- 0
...
#                      gamma_1*w10*delta + gamma_2*w11*delta + gamma_3*w10*delta + gamma_4*w11*delta + gamma_5*w10*delta + gamma_5*w11*delta )
#   
#   theta_A <- (x4^2)*theta_C
#   
#   theta_B <- 2*theta_C*x4*(nu0 + x1*w10*nu1 + x2*w11*nu2 + x3*nu3) + x4*theta_D
#   
#   mu0_sd <- sqrt( 1 / (sum(theta_A / theta_denom) + 1/k0 ) )
#   
#   mu0_mean <- (-1) * ( sum(theta_B / theta_denom) - 2*k1/k0 ) / ( 2 * ( sum(theta_A / theta_denom) + 1/k0 ) )
...
# draws <- 10000
# 
# nu0_t_draw <- nu0_cond_posterior(sigma0_true, sigma10_true, sigma11_true, nu1_true, nu2_true, nu3_true, nu4_true, delta_mean, x1, x2, x3, x4,  potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# 
# # nu1 draw
# nu1_t_draw <- nu1_cond_posterior(sigma0_true, sigma10_true, sigma11_true, nu0_true, nu2_true, nu3_true, nu4_true, delta_mean, x1, x2, x3, x4, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# 
# # nu 0 draw
# nu2_t_draw <- nu2_cond_posterior(sigma0_true, sigma10_true, sigma11_true, nu0_true, nu1_true, nu3_true, nu4_true, delta_mean, x1, x2, x3, x4,  potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
...
# 
# # nu 0 draw
# nu3_t_draw <- nu3_cond_posterior(sigma0_true, sigma10_true, sigma11_true, nu0_true, nu1_true, nu2_true, nu4_true, delta_mean, x1, x2, x3, x4, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
# nu3_mat <- data.frame(1:draws, nu3_t_draw)
# names(nu3_mat) <- c("draws", "nu3_vals")
# ggplot(nu3_mat, aes(draws, nu3_vals)) + geom_line()
# 
# 
# 
# # nu 0 draw
# nu4_t_draw <- nu4_cond_posterior(sigma0_true, sigma10_true, sigma11_true, nu0_true, nu1_true, nu2_true, nu3_true, delta_mean, x1, x2, x3, x4,  potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
# nu4_mat <- data.frame(1:draws, nu4_t_draw)
# names(nu4_mat) <- c("draws", "nu4_vals")
# ggplot(nu4_mat, aes(draws, nu4_vals)) + geom_line()
# 
# 
# 
# # update the mu0_t draw
# 
# mu0_true <- nu0_true + x1*potential_outcomes$w10*nu1_true + x2*potential_outcomes$w11*nu2_true + x3*nu3_true + x4*nu4_true
# 
# # delta
# delta_t_draw <- delta_cond_posterior(sigma0_true, sigma10_true, sigma11_true, mu0_true, potential_outcomes$y00, potential_outcomes$y10, potential_outcomes$y11, potential_outcomes$w10, potential_outcomes$w11, k0_mu0, k1_mu0)
# 
# 
